...
describe("GovernTBClient", () => {
	...
	describe("health check interval", () => {
		...
		it("exercises health check failure path when ping rejects (onAlert)", async () => {
			...
			const onAlert = vi.fn();
			const alertClient = new GovernTBClient({
				addresses: ["3000"],
				onAlert,
			});
		...
		it("exercises health check failure path when ping rejects (console.warn fallback)", async () => {
			...
			// No onAlert callback
			const noAlertClient = new GovernTBClient({ addresses: ["3000"] });
	...
	describe("_doReconnect error handling", () => {
		...
		it("reconnect deduplicates concurrent calls", async () => {
			client.destroy();
			const testClient = new GovernTBClient({ addresses: ["3000"] });
			testClient.destroy();
	...
	describe("constructor options", () => {
		...
		it("accepts custom clusterId", () => {
			const customClient = new GovernTBClient({
				addresses: ["3000"],
				clusterId: 5n,
			});
